...
class AiLUT(nn.Module):
    ...
    def ailut_transform(self,imgs,luts,vertices):
        ...
        Img = torch.zeros(vertices.size(0),imgs.size(2),imgs.size(3),self.n_colors)
        for n in range(vertices.size(0)):
            for color in range(self.n_colors):
                now = 0
                for i in range(self.n_vertices):
                    while (now <= vertices[n][color][i]*255): 
                        R[color][now] = i
                        now+=1
                        if (now > 255): break
                now = 255
                for i in range(self.n_vertices-1,-1,-1):
                    while (now >= vertices[n][color][i]*255):
                        L[color][now] = i
                        now -= 1
                        if (now <0): break
            ...
            for color in range(self.n_colors):
                for i in range(imgs.size(2)):
                    for j in range(imgs.size(3)):
                        l = L[color][(imgs[n][color][i][j]*255).int()].item()
                        r = R[color][(imgs[n][color][i][j]*255).int()].item()
                        #print(l,r)
                        Img[n][i][j][color] = (l + (imgs[n][color][i][j] - vertices[n][color][l])/(vertices[n][color][r]-vertices[n][color][l])) *2 / self.n_vertices -1
        #print(luts.shape,Img.shape)
        output = F.grid_sample(luts,Img.unsqueeze(1)).squeeze(2)
        return output
        '''
        ...
        for n in range(imgs.size(0)):
            for i in range(imgs.size(2)):
                print(i)
                for j in range(imgs.size(3)):
                    l0 = L[0][I[n][0][i][j].int()].item()
                    r0 = R[0][I[n][0][i][j].int()].item()
                    l1 = L[1][I[n][1][i][j].int()].item()
                    r1 = R[1][I[n][1][i][j].int()].item()
                    l2 = L[2][I[n][2][i][j].int()].item()
                    r2 = R[2][I[n][2][i][j].int()].item()
